...
class V2Ichannels: 
    ...
    def __init__(self, n_Veh, n_RB):
        ...
        self.BS_position = [[1750, 30],[5200,30],[8650,30]]    # Suppose the BS is in the center
        #self.BS_position = [1750, 30]   # Suppose the BS is in the center
        self.shadow_std = 8
        self.n_Veh = n_Veh
        self.n_RB = n_RB
        self.update_shadow([])
    ...
    def update_pathloss(self):
        ...
        for i in range(len(self.positions)):
            x_dist_BS_temp = 0
            used_BS = [0,0]
            for j in range(len(self.BS_position)):
                x_dist_BS_temp=self.positions[i][0] - self.BS_position[j][0]
                if(x_dist_BS_temp<0):
                    x_dist_BS_temp = -1 * x_dist_BS_temp
                if((x_dist_BS_temp < used_BS[0])or(used_BS[0]==0)):
                    used_BS = self.BS_position[j]
                
            d1 = abs(self.positions[i][0] - used_BS[0])
...
class Environ:
    ...
    def create_platoons(self):
        ...
        while vehicles:
            platoon_size = random.randint(1, min(5, len(vehicles)))
            platoon = random.sample(vehicles, platoon_size)
            self.platoons.append(platoon)
            for vehicle in platoon:
                vehicles.remove(vehicle)
        self.platoon_sizes = [len(platoon) for platoon in self.platoons]
    ...
    def assign_vehicles_to_platoons(self):
        # Assign vehicles to different platoons
        self.platoons = []
        vehicle_index = 0
        for size in self.platoon_sizes:
            if vehicle_index + size <= self.n_Veh:
                self.platoons.append(list(range(vehicle_index, vehicle_index + size)))
                vehicle_index += size
            else:
                break  # Stop if we run out of vehicles    
        ...
        while(i < len(self.vehicles)):
            ...
            #    print ('delete ', self.position[i])
                        if (self.vehicles[i].direction == 'l'):
                            ...
                            self.vehicles[i].position = [self.vehicles[i].position[0],self.right_lanes[0]]
                        else:
    ...
    def act_for_training(self, actions, idx):
        ...
        # Assuming actions is an array of actions taken by the agent
